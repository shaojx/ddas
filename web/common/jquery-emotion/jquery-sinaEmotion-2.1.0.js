...
(function($) {
	...
	var initEvents = function() {
		...
		$('#sinaEmotion').bind({
			...
			click: function(event) {
                ...
                $target.append($(this).children('img').clone());
				event.preventDefault();
			}
		});
	};
	...
	var loadEmotions = function(callback) {
		...
		$.getJSON(path+'/common/jquery-emotion/emotions.json', {}, function(json) {
			var item, category;
			var data = json.data;
            var categoryCount=1;
			...
			for (var i = 0, l = data.length; i < l; ++i) {
                ...
                //convert category
                if(category!=defCategory&&local!="zh_CN"){
                    if(!mapping[category]){//new one
                        var oleCategory=category;
                        category=emojiPre+categoryCount;
                        mapping[oleCategory]=category;
                        emotions[category]=[];
                        categories.push(category);
                        categoryCount++;
                    }else{
                        category=mapping[category];//get mapping
                    }
                }else{//zh_CN
				...
				var icon=item.icon;
                if(item.icon.lastIndexOf("/")!=-1){
                    icon=path+"/common/jquery-emotion/icons"+item.icon.substring(item.icon.lastIndexOf("/"));
                }
				emotions[category].push({
					icon:icon ,//parse to local address(not sina host)
					//icon:item.icon ,
					phrase: item.phrase
				});

				//emotionsMap[item.phrase] = item.icon;
				emotionsMap[item.phrase] = icon;
			}
	...
	$.fn.getSubmitText=function(){
        var _target=$target||$(this);
        var html=_target.html();
        html=html.replace(/(<img.*?>)/g,function($1){
       ...
       /* var imgs=$target.find("img");
        $.each(imgs,function(index,ele){
            $(this).prop("alt").insertBefore($(this));
            $(this).remove();
        });*/
       //
    }
